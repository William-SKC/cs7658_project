...
class A1andLaikagoGaitProblem:
    def __init__(self, rmodel, lfFoot, rfFoot, lhFoot, rhFoot):
        ...
        q0 = np.array([0., 0., 0.42, 0, 0, 0, 1, 0, 0.6, -1.15, 0, 0.6, -1.15, 0, 0.6, -1.15, 0, 0.6, -1.15])
    ...
    def createBackFlippingProblem(self, x0, jumpHeight, jumpLength, timeStep, groundKnots, flyingKnots):
        ...
        base_task = np.array([0., 0., 0.42, 0, 1, 0, 0, 0, 0.6, -1.15, 0, 0.6, -1.15, 0, 0.6, -1.15, 0, 0.6, -1.15])
        backflipPhase = [
            self.createSwingFootModel(
                timeStep, [], [], [],
                baseTask=base_task
    ...
    def createSwingFootModel(self, timeStep, supportFootIds, comTask=None, swingFootTask=None, baseTask=None):
        ...
        for i in supportFootIds:
            supportContactModel = crocoddyl.ContactModel3D(self.state, i, np.array([0., 0., 0.]), self.actuation.nu,
                                                           np.array([0., 50.]))
        ...
        if swingFootTask is not None:
            for i in swingFootTask:
                ...
                #print(i)
                #frameTranslationResidual = crocoddyl.ResidualModelFrameTranslation(self.state, i.id, i.placement.translation,
                #                                                                   self.actuation.nu)
        ...
        if baseTask is not None:
            baseRef = np.concatenate([baseTask, np.zeros(self.rmodel.nv)])
            baseWeights = np.array([0.] * 3 + [1.] * 3 + [0.] * (self.rmodel.nv - 6) + [0] * 6 + [0] *
                                   (self.rmodel.nv - 6))
            baseReg = crocoddyl.CostModelState(self.state, crocoddyl.ActivationModelWeightedQuad(baseWeights ** 2),
                                               baseRef, self.actuation.nu)
            costModel.addCost("baseReg", baseReg, 1e8)

        stateWeights = np.array([0.] * 2 + [0.] * 1 + [0.] * 3 + [0.01] * (self.rmodel.nv - 6) + [10.] * 6 + [1.] *
                                (self.rmodel.nv - 6))
